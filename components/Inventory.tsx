...
const Inventory: React.FC<InventoryProps> = ({ vehicles, onAddVehicleClick, onDeleteVehicle }) => {
  return (
    <div className="p-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div className="relative w-full max-w-xs">
          <input type="text" placeholder="Search inventory..." className="w-full bg-slate-200 dark:bg-slate-700 p-3 pl-10 rounded-lg text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors duration-300" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {vehicles.map(vehicle => <VehicleCard key={vehicle.id} vehicle={vehicle} onDelete={() => onDeleteVehicle(vehicle.id)} />)}
      </div>
    </div>
  );
};
